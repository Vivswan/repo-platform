...
export function fleetTokenHolderMismatches(workflows: Record<string, string>): Mismatch[] {
  ...
  for (const rel of holders) {
    if (!classified.includes(rel)) {
      mismatches.push({
        file: rel,
        expected:
          "a fleet-token holder registered in check_ssot.ts: FLEET_WRITERS (it mutates managed repos - then it rides post-green) or FLEET_TOKEN_NON_WRITERS (with why it holds the token)",
  ...
  for (const rel of classified) {
    if (!holders.includes(rel)) {
      mismatches.push({
        file: `scripts/check_ssot.ts ${rel in FLEET_WRITERS ? "FLEET_WRITERS" : "FLEET_TOKEN_NON_WRITERS"}`,

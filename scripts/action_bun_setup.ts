// The composite actions' bun setup: the steps every bun-running action.yml
// under actions/ opens with (pin probe, pinned install, one retry, the
// recorded bun path), generated by scripts/generate.ts into a fenced
// region of each manifest and held there by generate:check. One source
// because a relative `uses:` inside a composite action resolves against
// the CALLER's workspace, so the steps cannot be hoisted into a shared
// action; the actions-bun-guard ssot rule demands the region of every
// action that runs bun, so no action ships them by hand.

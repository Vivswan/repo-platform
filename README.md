...
| Managed + local sections | `.gitignore` (LOCAL section is yours), `SECURITY.md`, `CONTRIBUTING.md` (everything below the `<!-- repo-platform:local-section -->` line is yours; conflict resolution moves overlapping local edits below it - where there is local content to move - instead of dropping them, and they still appear in the PR body for review; both files are public-only renders, so a flip to private retires the whole file, local section included) |

...
Repository settings are not part of that render: for every managed repo (one with a `.repo-platform.yml`), [settings-repos.yml](.github/workflows/settings-repos.yml) selects each repo's settings layers at apply time - fleet baseline, fleet visibility overlay, the selected modules' layers and their visibility overlays, the tracking labels, the repo's own `.github/settings.yml`, then a fleet override layer no repo can weaken - and hands the ordered list to github-settings-as-code, whose `mode: merge` folds them into the document it then applies ([docs/settings.md](docs/settings.md)).

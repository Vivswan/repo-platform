...
- Then `copier copy /tmp/bt /tmp/out --vcs-ref HEAD --defaults --trust -d project_name=X -d description=Y -d 'modules=[uv]' -d private=false` and run `bun actions/validate-template/validate_generated_files.ts /tmp/out` (the validator is a script directory, not an action; validate-template-report runs it). The multiselect value must be a YAML list in ONE `-d` argument.

...
- A file the platform stops writing gets a `retired` entry in `files.yml` (the sync deletes it); a rename is a retired entry for the old path plus the new entry. The transitional code is the writer's cutover of pre-writer repositories (.github/scripts/sync/writer/cutover.ts) and fleet-ci.yml's ignored legacy inputs, both retired at the fleet cutover.

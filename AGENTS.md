...
- A file the platform stops writing gets a `retired` entry in `files.yml` (the sync deletes it); a rename is a `retired` entry carrying `moved_to` for the old path plus the new entry.

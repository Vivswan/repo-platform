...
- `template/` is GENERATED by `scripts/compose_template.py` - never edit it
  by hand; run `bun run compose` after changing `templates/` (CI's
  compose-fresh job fails on drift). Copier renders downstream repos from
  this composed tree; per-module parameters (pages_*) are asked only when
  their module is selected.
...
- Never hand-edit `template/` (composed) or `templates/base/.gitignore.jinja`
  (generated); run `python3 scripts/build_gitignore.py` (or `--locked`) then
  `bun run compose`. CI fails on drift from either generator.
...
- `bun run check` chains every local gate, including `compose:check`
  (template/ matches templates/) and `gitignore:check`.

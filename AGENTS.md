...
- Fleet config (which repos, which channel) is `repos.yml`; module selection lives in each repo's own `.repo-platform.yml`; the managed settings baseline is assembled per repo at apply time by `.github/scripts/fleet/render_managed_settings.ts` from `.github/settings-baseline.yml` plus the module manifests, and each repo's own `.github/settings.yml` deep-merges over it (`merge_settings_layers.ts`). `_skip_if_exists` in copier.yml is the generated-once-then-repo-owned list.

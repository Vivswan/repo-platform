...
- Composition rules (gates, `{# compose:<name> #}` anchors, fragments, collisions): the header of scripts/compose_template.ts. Fleet membership: `repos.yml`. Module selection: each repo's `.repo-platform.yml`. Settings: the six-layer merge in docs/settings.md.
...
- A template change that renames a rendered file, retires one, or flips its ownership class ships an upgrade_path_test.sh case and a PR-body note; the sync pipeline carries the transition, and no version-gated migration system exists.

...
- Repository settings are rendered into `.github/settings.yml` by the sync from the fleet layers plus this repository's own `.github/settings.local.yml`. Edit that file, never the rendered one or the GitHub UI; the merge rules are in the platform's docs/settings.md.
...
- Module selection is the `modules` list in `.repo-platform.yml`; the next sync PR applies a change. The per-module contracts are in the platform's docs/new-repo.md.

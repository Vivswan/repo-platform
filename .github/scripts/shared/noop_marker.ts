...
// whose render leaves the build branch byte-identical. The append-only
...
// as a no-op against build tip Y". publish.ts writes the message and
...
// the build branch itself. The marker therefore carries NO authority:
 ...
 * checkout, no branch protection semantics) - it carries no authority, so
 * unlike the refs/heads/build-pending/ parking refs it has no ruleset
 * option to keep. Per-source like pending refs: concurrent
...
export const NOOP_MARKER_REF_PREFIX = "refs/build-meta/build-noop/";
...
export function noopClaimName(sourceSha: string, tipSha: string): string {
  ...
  return `build-noop-${sourceSha}-${tipSha}`;
}

/** The full marker commit message: the standard source/run stamp lines
 * plus a tip line binding the claim to the exact build tip it was
...
export function noopMarkerMessage(
  ...
  return [
    `build(build): source ${sourceSha.slice(0, 12)} is a no-op against tip ${tipSha.slice(0, 12)}`,
    "",
...
/** The marker's bound build tip sha (empty when there is none). */

// Rebuild a channel tree from a source commit exactly as the builder does:
// the SOURCE commit's own script and dependencies, so the rebuild
...
/** Build the channel tree from `sourceSha` into `treeDir` (via a detached
...
export function rebuildChannelTree(options: {
  sourceSha: string;
  channel: string;
  version?: string;
  srcDir: string;
  ...
  const { sourceSha, channel, version, srcDir, treeDir } = options;
  ...
  step(version !== undefined ? [...build, "--version", version] : build);

...
# Each probe answers one question about one repo. Return codes: 0 = the
# repo passes, 10 = a definitive negative (the probe already explained
# it), 1 = no answer this attempt (transient or unexpected failure;
# $probe_error carries the detail for the retry loop).
probe_error=""
...
probe_push() {
  ...
  case "$code" in
    200) return 0 ;;
    401 | 403 | 404)
      echo "::notice::${1}: skipped - the fleet token has no write access (push probe HTTP ${code}). Grant the REPO_PLATFORM_TOKEN access to this repository to enroll it, or add it to repos.yml's exclude list to silence this."
      return 10
      ;;
    *)
...
probe_settings() {
  ...
  if grep -q "HTTP 404" "$RUNNER_TEMP/probe.err"; then
    ...
    warn "$1 is enrolled and adopted but has no settings home: no settings/repos/$name.yml here and no .github/settings.yml in the repo. Its settings are unmanaged - nothing installs or heals the main ruleset (so all-green may not be a required check) and labels are never reconciled. Pick a home per docs/settings.md."
    return 10
  fi
  probe_error="$(cat "$RUNNER_TEMP/probe.err")"
  return 1
...
# probe <label> <fn> <repo>: rc 0 keeps the repo in the pipeline, rc 1
# drops it - either a definitive negative (already reported by the probe)
# or still no answer after the retries, which warns loudly: a silently
# dropped repo would heal nothing tonight and nobody would know.
attempts=3
probe() {
  local label="$1" fn="$2" repo="$3" rc try
  for try in $(seq 1 "$attempts"); do
    ...
    if [ "$try" -lt "$attempts" ]; then
      echo "${repo}: ${label} failed (attempt ${try}/${attempts}: ${probe_error}); retrying..."
      sleep 5
    fi
  done
  warn "${repo}: the ${label} failed ${attempts} times (last error: ${probe_error}) - not a permission or adoption answer, so the repo is skipped this run; the nightly heal retries it. If this persists, check the repo's availability and the fleet token."
  return 1
}
...
while IFS=$'\t' read -r repo name; do
  ...
  probe "push-permission probe" probe_push "$repo" || continue
  probe "adoption check" probe_adoption "$repo" || continue
  probe "settings.yml check" probe_settings "$repo" || continue
  repos="$repos$repo"$'\n'

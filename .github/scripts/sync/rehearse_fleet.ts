...
async function main(): Promise<number> {
  ...
  // actions/validate-template-report/validator/node_modules on a fresh checkout is
  ...
  if (install.exitCode !== 0) {
    fail(
      `installing the validator's deps (actions/validate-template-report/validator) failed: ${install.stderr.trim().split("\n").pop() ?? ""}`,
    );
  }

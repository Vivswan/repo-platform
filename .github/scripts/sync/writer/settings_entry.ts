...
/** Whether `text` is a document this render wrote, by its first line: the
 *  judgment the displacement pass and the settings apply's selector
 *  (fleet/select_settings_repos.ts) share. */
export function isRendered(text: string): boolean {
  return text.split("\n", 1)[0] === RENDERED_HEADER;
}

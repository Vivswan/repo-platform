...
// checked against the template ref) into $RUNNER_TEMP/modules.json - a
...
// for them and printed as ::error:: commands otherwise.
//
...
console.log(
  hideDetails()
    ? `selected modules: ${kept.length} (names hidden: private repository)`
    : `selected modules: ${JSON.stringify(kept)}`,
);

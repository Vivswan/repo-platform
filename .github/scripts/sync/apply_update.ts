...
/** The commit TARGET_REF names, resolved once before copier runs and handed
 * to both copier and the postcondition. Candidates in order: SRC_PATH, the
 * answers file's local `_src_path`, then cwd. */
function targetSha(): string {
  // Lazy: a later candidate's failure must not abort an earlier answer.

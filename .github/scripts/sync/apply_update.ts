...
/** The commit TARGET_REF names, resolved ONCE before copier runs and
 * handed to both copier (`--vcs-ref`) and the postcondition, so a branch
 * or tag that moves between the two cannot fail a legitimate render.
 * Resolved in the first repository that knows it: SRC_PATH, the answers
 * file's `_src_path` when that is a local directory (the harness and the
 * rehearsal point it at their build clone), then the cwd (the sync's
 * checkout, where resolve_refs fetched the build branch). The production
 * ref is already a sha and resolves anywhere it exists; a tag or branch
 * name (the harnesses) resolves only where it lives. */
function targetSha(): string {
  // Evaluated in order, lazily: a later candidate's own failure (an
  // unreadable recorded _src_path) must not abort a resolution an earlier
  // one answers.
  const candidates = [() => env("SRC_PATH"), recordedSrcPath, () => "."];
  const tried: string[] = [];
  ...
  return fail(
    `cannot resolve TARGET_REF '${targetRef}' to a commit in ${tried.join(", ")}, so the recorded _commit cannot be verified`,
  );
}
...
const mismatch = recordedCommitMismatch(recorded, sha);

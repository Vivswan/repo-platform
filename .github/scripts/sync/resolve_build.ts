...
if (fetched.exitCode !== 0) {
  fail(
    `fetching the stable tag failed (${lastLine(fetched.stderr) || `exit ${fetched.exitCode}`}). ${remove}`,
  );
}
...
if (!gitAnswersYes(["merge-base", "--is-ancestor", tip, MAIN])) {
  fail(`the stable tag names ${short}, which is not on main's history. ${remove}`);
}
const notGreen = allGreenFailure(repository, tip);
if (notGreen !== null) {
  fail(`the stable tag names ${short}, which is not green: ${notGreen}. ${remove}`);
}
...
if (!gitAnswersYes(["rev-parse", "--verify", "--quiet", `${tip}:${FILES_CONFIG}`])) {
  fail(
    `the stable tag names ${short}, which carries no ${FILES_CONFIG}: the writer's data file is not at that commit's root, so there is nothing to sync from. ${remove}`,
  );
}
setOutput("build", tip);

...
Any template change that renames a rendered file, retires one, or flips its ownership class (template-managed <-> repo-owned, e.g. a path entering or leaving `_skip_if_exists`) MUST ship three things in the same PR: the `migrations/<from-version>.ts` script, an upgrade-path test case (below), and a PR-body note naming the transition. `scripts/check_migrations.ts` (in `bun run check`) is the tripwire for the forgotten script: it compares the newest release tag's rendered landing paths against the working tree's composed template and fails naming the retired path and the expected migration filename.

// The slice of the repository's shared helpers this action needs, kept
// LOCAL on purpose. A composite action is copied to the template branch and
// runs from its own directory, so it cannot import out of the repository
// tree. That is a property of how actions are published, not drift: the gate
// PREDICATE exists exactly once, in gate.ts, and the operator's ci.yml calls
// this action rather than keeping a second copy of it.
//
// Keep these behaviour-compatible with .github/scripts/shared/ - they are
// the same functions, narrowed to what the gate uses.

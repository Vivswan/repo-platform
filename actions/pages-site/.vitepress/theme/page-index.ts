// URL rules come from derive.ts, never restated here. The headers come out of a full render (headersRule stamps them
// on the env), so VitePress's own pipeline, frontmatter stripping included, is what decides which headings exist.
...
/** Runs last in core, after the anchor plugin has assigned heading ids. */
...
/** Throws when the renderer lacks headersRule: the stamp is the contract. */
...
/** Only top-level headings: one nested in a quote or list is not an outline entry, as VitePress's headers plugin also decides. */
...
/** `indexPages` are the include roots' page files (derive.ts's includeIndexPages), served at their directory URLs. */

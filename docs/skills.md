...
```
skills/             # the skills directory (skills.dir in .repo-platform.yml, default `skills`)
...
- The two `.claude-plugin/` manifests are starters: written once, then repo-owned. Repos list their published skills in `plugin.json`'s `skills` array and tailor the descriptions; the sync never overwrites that. A repo adopting the module with existing manifests keeps them untouched.
- The fleet-ci `validate-skills` job and the advisory `validate-skills.yml` are managed: validation logic updates reach the whole fleet through the `@build` refs with no sync PR, and the advisory workflow file itself updates on sync. Do not edit them; repo-specific checks belong in the repo-owned `checks.yml`.
...
## The fleet-operations skills

repo-platform hosts three skills under [`skills/`](https://github.com/Vivswan/repo-platform/tree/main/skills) for driving the platform from other repositories. They are not platform files: the sync never writes them into managed repos.

| Skill | What it tells the agent to do |
|---|---|
| `repo-platform-new-project` | Create the repo, write `.repo-platform.yml` (only the keys whose defaults are wrong), grant the fleet token, dispatch the first sync with `manual=true`, read the sync PR's report, fill in the starters, watch the first CI run |
| `repo-platform-add-module` | Edit `modules:` and the module's keys in the registration, merge (the plan job validates it), run the sync for the module's files, check the report against the module diff, finish the companion steps; which files each module brings, and that `ci.yml` never changes |
| `repo-platform-sync-pr` | Read a sync PR's report (Written, Replaced local edits, Retired, Registration notes, Mirrors, Review) and its hold reasons, handle the manual review cases, keep local content in the repo-owned tail or a repo-owned hook, follow the `[repo-platform] sync failed` issue |
...
1. Add `skills` to the `modules` list in `.repo-platform.yml`; the next sync writes the module's files.
2. The existing `.claude-plugin/` manifests survive untouched (a starter is written only when absent), catalog included.

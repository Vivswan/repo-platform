...
- **Images:** an image in the article opens in a lightbox on click, or on Enter once focused, enlarged to fit the viewport (a raster image no further than its natural size); Escape, a click, or scrolling away closes it. A linked image stays a link, and an image with an empty alt is decorative and stays put.

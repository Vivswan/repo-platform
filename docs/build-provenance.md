...
The delivery is the full 40-hex sha of that main commit: the writer takes it from the operator's `--build` argument (the commit resolve_build.ts resolved for the whole run) and names it in the sync commit's subject and the PR body, so every repository's history names the exact commit its files came from; the tree itself carries no build stamp, so an unchanged tree opens no PR ([sync.md](sync.md#the-manifest)). Old delivery commits stay reachable forever: they are main history.

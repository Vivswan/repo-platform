...
- A managed repository's ci.yml is one byte-identical file for the whole fleet. It carries three gating jobs: `checks` (calls the repo-owned checks.yml; skipped on the nightly schedule run, which the two fleet callers share: in the `ci` caller CodeQL reruns on the plan's `weekly` day and the other gate jobs stand down, and the `nightly` caller runs the Trivy scan and files its tracking issue), `ci` (calls [fleet-ci.yml](../.github/workflows/fleet-ci.yml)`@build` with no inputs: its first job `plan` reads `.repo-platform.yml` through [actions/plan](../actions/plan/action.yml) and outputs the module selection, visibility, and labels), and `all-green` needing both - plus the gate-downstream `post-green` caller and the static legs ([after the gate](#after-the-gate)), and the schedule-only `nightly` caller of [fleet-nightly.yml](../.github/workflows/fleet-nightly.yml)`@build` (the nightly [security scan](security-scans.md), split out because its `issues: write` grant exceeds the `ci` caller's ceiling), all of which gate nothing. The membership rule: what gates a managed repository is being a job in fleet-ci.yml or checks.yml - a caller job's result aggregates every job of the workflow it calls, so a failure anywhere inside fails the gate.
...
| `fleet-ci-roster` | fleet-ci.yml's job set (`plan`, `validate-template`, `base-checks`, `dependency-review`, `zizmor`, `knip`, `semgrep`, `codeql`, `validate-skills`, `release-freshness`, `release-health`, `trivy`), both directions - deleting `codeql` there would drop the gate for every managed repository at once - and no job-level `continue-on-error` (a softened fleet job would read green to every caller's gate; `file-size` softens its own step inside `base-checks`). |
...
The rendered ci.yml itself needs no rule of its own: it carries no module- or visibility-conditioned text, so the smoke matrix proves every selection renders the same bytes as the committed `minimal` golden ([golden-renders.md](golden-renders.md)), and the validator's gate check judges the file in every repository.
...
- Repo-platform's own docs site is the skeleton's docs-site leg, carried by hand in its ci.yml (that file is not rendered): a `docs-site` job ordered behind `post-green`, so the deploy runs after this run republished the `build` branch its pipeline consumes and ships the theme that publish landed. It calls reusable-pages.yml by local path with its mount spelled out, since this repository carries no registration for the pipeline to read.
...
- The block is public text (the pull request's body, or a direct push's message on `main`), and so is every log line that echoes it: private repositories appear there only as the word `private`; the plans print them by their masked hint.

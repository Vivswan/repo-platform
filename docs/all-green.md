...
- The `release` leg needs the gate AND the hook, so the repo's post-green work lands before the tag is minted, then calls [fleet-release.yml](../.github/workflows/fleet-release.yml)`@build` with the judged sha and fleet-ci's `tracking-labels` output, holding the `post-green-release` lane; release-please tags the merged release PR's own merge commit, never the branch head, and only a run whose judged commit is a release-PR merge tags (release-health's `release-cut` output skips the tag phase on every other push run), so the release lands with the gate that judged it whatever main does afterwards; a release merge whose own run never finished (cancelled by the caller's concurrency group or red) stays pending until that run is re-run, and the stale-label guard names it on the next push. Its outputs drive the release hooks: `update-release` (the repo-owned update-release.yml with the tag), `publish-release` (fleet-release-publish.yml@build: attests the assets and flips the draft live), and `update-release-pr` (the repo-owned twin when a release PR was created or refreshed). The two hooks are universal starters, seeded in every repository whatever its modules, because GitHub resolves a called `./` workflow at run creation whether or not the job's condition holds ([new-repo.md](new-repo.md#the-release-pipeline-release-please)).

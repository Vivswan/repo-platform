...
- Beside it: `validate-template`; `module-render` (pull requests only: a PR that changes the module selection must carry its render, [new-repo.md](new-repo.md#changing-the-module-selection)); `zizmor` (every visibility, its SARIF upload on public repositories only); `knip` (bun or node repositories); `semgrep` and `dependency-review` (public repositories only); `codeql`; and the module jobs. Each check's per-finding bypass is the table in [fleet-guidelines.md](fleet-guidelines.md#how-to-bypass-a-check).

...
- Beside it: `validate-template`; `module-render` (pull requests only: a PR that changes the module selection must carry its render, [new-repo.md](new-repo.md#changing-the-module-selection)); `zizmor` (every visibility, its SARIF upload on public repositories only); `knip` (bun or node repositories; a repository with no package.json to install from yet, or no npm lockfile for `npm ci`, stands down with a notice); `semgrep` and `dependency-review` (public repositories only); `codeql`; and the module jobs. Each check's per-finding bypass is the table in [fleet-guidelines.md](fleet-guidelines.md#how-to-bypass-a-check).
...
| `fleet-ci-roster` | fleet-ci.yml's job set (`plan`, `validate-template`, `module-render`, `base-checks`, `dependency-review`, `zizmor`, `knip`, `semgrep`, `codeql`, `validate-skills`, `release-freshness`, `release-health`, `trivy`), both directions - deleting `codeql` there would drop the gate for every managed repository at once - and no job-level `continue-on-error` (a softened fleet job would read green to every caller's gate; `file-size` softens its own step inside `base-checks`). |

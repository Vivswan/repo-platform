...
| `pages-leg` | The pages module's leg, the same leg model: its condition block (gate alone), judged-sha pass, `pages` lane, permissions ceiling, and no secrets; the called pages.yml with no `push` trigger, the `sha` input declared and handed on, and its lane keyed per called run; reusable-pages.yml's checkout reading the sha first; and the all-modules golden render, parsed: pages.yml's trigger set is exactly workflow_call, schedule, and dispatch, and the rendered `pages` job needs the gate alone and passes the judged sha. |
...
- On pages repos, the rendered ci.yml's `pages` leg needs the gate alone and calls the managed pages.yml with the judged sha, holding the `pages` lane ([pages.md](pages.md)); pages.yml has no `push` trigger of its own, only the nightly rebuild and a dispatch, which build the default branch head. The leg is independent of the hook and the release leg: a red hook never holds the site back, and a tag minted in the same run lands on the nightly rebuild.

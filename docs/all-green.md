...
- Beside it: `validate-template`; `zizmor` (every visibility, its SARIF upload on public repositories only; each of its two passes runs once more when the first attempt fails, because zizmor exits alike on a finding and on an error and one transient GitHub API error in its online known-vulnerable-actions audit has failed a run, so a high finding fails both judging attempts and only a transient error passes the retry); `knip` (bun or node repositories; a repository with no package.json to install from yet, or no npm lockfile for `npm ci`, stands down with a notice); `semgrep` and `dependency-review` (public repositories only); `codeql`; and the module jobs. Each check's per-finding bypass is the table in [fleet-guidelines.md](fleet-guidelines.md#how-to-bypass-a-check).
...
| `fleet-caller-ceilings` | Every job grant in fleet-ci.yml and fleet-nightly.yml under its skeleton caller's (`ci` and `nightly` in the rendered ci.yml): GitHub checks a nested job's permissions against the caller's when the call is expanded, before the job's `if:` runs, so one scope over the ceiling fails every fleet run at once. |

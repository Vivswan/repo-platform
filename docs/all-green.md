...
- The mention scan reads each line behind its leading whitespace and blockquote markers, in any mix, and a line that is only markers (`>`) is a paragraph break; a mention is bare when either reading of a blockquote start leaves it bare, spans pairing across the `>` (a lazy continuation) or ending there (a `>` interrupts a paragraph), so a blockquote start the body did not have never hides a mention the pairing-across reading shows: a fenced example quoted with `> ` is still a fence and a quoted code span is still a span. The block itself is written at the start of the line, unquoted and unindented; a quoted or indented directive is a misplaced directive, and an error quotes the line as written.

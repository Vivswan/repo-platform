...
- Every shape above is read behind the line's leading whitespace and blockquote markers, in any mix: a fenced example quoted with `> ` is still a fence, a quoted block line is still a block line (the block itself may be written in a blockquote), and an error still quotes the line as written.

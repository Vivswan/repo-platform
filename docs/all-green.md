...
- The block is public text on `main`, and so is every log line that echoes it: private repositories appear there only as the word `private`; the sync plan counts them and the settings plan prints a hint.

...
- When the publish ADVANCED the tip, the same job dispatches this repository's own [docs-site.yml](../.github/workflows/docs-site.yml) on main. That site consumes `@build` like every fleet site, and its push-triggered deploy starts before the publish lands, so without the redeploy a commit changing the theme shipped the previous theme. The push-triggered deploy may still run first and be overwritten by the dispatched one (both hold the `pages` lane, so they never overlap); a skipped or no-change publish dispatches nothing. The dispatch rides the fleet PAT, since ci.yml's caller caps `GITHUB_TOKEN` at `contents: read` and a called job cannot rise above it.
...
checks + ci -> all-green -> post-green (repo-owned hook) -> release (release-please module) -> pages (pages module, ordered behind release only where it exists)
                        \-> pages (pages module, without release-please)
```

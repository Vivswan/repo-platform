...
- The parser scans the WHOLE merged message. A mention wrapped in a code span (`` `[fleet-sync: public]` `` in prose: a backtick run closed by the next run of the same length, as in CommonMark) is prose, so describe the grammar that way. A span may cross line breaks inside its paragraph (GitHub wraps the squash body at 72 columns, so a long span in the PR body arrives as several lines) but never a blank line or a fence line; a run without its closer is not a span, a fence line's backticks open a code block rather than a span, and a bare `[fleet-sync` anywhere else, even inside a fenced code example, is a misplaced directive.

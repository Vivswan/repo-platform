...
- Lost only when the merge's whole CI run is evicted by two later pushes (one pending run per branch): the evicting push's own run publishes the newer build, the weekly sync cron heals the sync leg, and the nightly settings cron heals the settings leg.

...
- The parser scans the WHOLE merged message. A mention wrapped in a code span (`` `[fleet-sync: public]` `` in prose) is prose, so describe the grammar that way; a bare `[fleet-sync` anywhere else, even inside a fenced code example, is a misplaced directive.

...
- Release health is enforced twice on purpose. The ci.yml `release-health`
  ...
  ordinary main pushes and release-PR refreshes are never blocked. Adding
  `release-override` to the release PR does not re-trigger CI (ci.yml does
  not run on `labeled`): re-run the failed release-health job after
  labeling; the release-time pre-flight reads the label on its own.

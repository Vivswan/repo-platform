...
- A managed repository's ci.yml carries three gating jobs: `checks` (calls the repo-owned checks.yml), `ci` (calls [fleet-ci.yml](../.github/workflows/fleet-ci.yml)`@build`, whose first job `plan` resolves the module selection, visibility, and labels from the repository's registration through [actions/plan](../actions/plan/action.yml); the inputs today's rendered ci.yml still passes are declared but ignored), and `all-green` needing both - plus the gate-downstream `post-green` caller and the module legs ([after the gate](#after-the-gate)), which gate nothing. The membership rule: what gates a managed repository is being a job in fleet-ci.yml or checks.yml - a caller job's result aggregates every job of the workflow it calls, so a failure anywhere inside fails the gate.
- Inside fleet-ci.yml, module- and visibility-conditioned jobs skip via job-level `if:` when they do not apply; a skipped job leaves the called run green. The base checks there (`typography`, `file-size` ([new-repo.md](new-repo.md#file-size-caps)), `commit-names`, `actionlint`, `yamllint`, and `gitleaks`) are the steps of one `base-checks` job for every visibility: each step runs under `!cancelled()` so one failure never hides another (a cancelled run stops them), and the last step fails the job naming every failed check in the log and step summary. It sits beside `validate-template`, `module-render` (pull requests only: a PR that changes the module selection must carry its render, [new-repo.md](new-repo.md#changing-the-module-selection)), and the module jobs. Repo-platform's own ci.yml has no callers to hide behind: its gating jobs are the needs list itself.
...
| `fleet-ci-roster` | fleet-ci.yml's job set (`plan`, `validate-template`, `module-render`, `base-checks`, `dependency-review`, `codeql`, `validate-skills`, `release-freshness`, `release-health`), both directions - deleting `codeql` there would drop the gate for every managed repository at once - and no job-level `continue-on-error` (a softened fleet job would read green to every caller's gate; `file-size` softens its own step inside `base-checks`). |
| `fleet-ci-render-roster` | The rendered ci.yml's shape at [the source](https://github.com/Vivswan/repo-platform/blob/main/templates/base/.github/workflows/ci.yml.jinja): exactly the `checks`/`ci`/`all-green`/`post-green` jobs plus the three leg anchors, the gate's exact lines, the post-green caller's condition block, judged-sha pass, `contents: read` ceiling, and absence of a lane, and the release leg's condition block (gate AND hook), judged-sha pass, and concurrency lane. |
| `pages-leg` | The pages module's leg, the same leg model: its condition block (the gate, led by `!cancelled()` under the release-please jinja gate), its needs list (the gate, plus the `release` edge under the same jinja gate, the only jinja the fragment may carry and exactly once each), judged-sha pass, `pages` lane, permissions ceiling, and no secrets; the called pages.yml with no `push` trigger, the `sha` input declared and handed on, and its lane keyed per called run; reusable-pages.yml's checkout reading the sha first; and two golden renders, parsed: pages.yml's trigger set is exactly workflow_call, schedule, and dispatch, the all-modules `pages` job needs the gate and `release` with the `!cancelled()` condition, and the pages-no-release-please `pages` job needs the gate alone under the plain condition, both passing the judged sha. |
| `docs-site-leg` | The docs-site module's leg in both arms its fragment renders (with and without the release-please edge): the gate clause under `!cancelled()`, the hook and release edges as order edges only, judged-sha pass, `pages` lane, permissions ceiling, and no secrets; the called docs-site.yml keeping its pull_request check, declaring the `sha` input under its workflow_call, keying its lane per called run, handing the sha on, and carrying no `push` trigger; the `docs-site-release-please` golden's rendered leg and callable workflow; the all-modules golden (pages too) rendering neither; and this repository's hand-written ci.yml `docs-site` job equal to the fragment's release-free arm. |
...
- Repo-platform's own docs site is the docs-site module's leg, carried by hand in its ci.yml (that file is not rendered): a `docs-site` job ordered behind `post-green`, so the deploy runs after this run republished the `build` branch its pipeline consumes and ships the theme that publish landed. The `docs-site-leg` rule holds the job equal to the module's fragment.
...
- On pages repos, the rendered ci.yml's `pages` leg needs the gate and calls the managed pages.yml with the judged sha, holding the `pages` lane ([pages.md](pages.md)); pages.yml has no `push` trigger of its own, only the nightly rebuild and a dispatch, which build the default branch head. With release-please selected too, the leg also needs `release`, as an ORDER and not a gate: its condition leads with `!cancelled()`, so the deploy waits for the release leg and then runs whatever that leg's result (a red hook skips the release; the deploy still runs). A release commit's own deploy therefore serves its new tag, where before the tag landed on the nightly rebuild.
- On docs-site repos without the pages module, the rendered ci.yml's `docs-site` leg calls the managed docs-site.yml with the judged sha under the same `pages` lane ([docs-site.md](docs-site.md)). It needs the hook, and the release leg when release-please is selected, as ORDER edges only: its condition opens with `!cancelled()` and gates on the all-green result alone, so a red or skipped hook or release never holds the site back while a tag minted in the same run lands on this deploy. docs-site.yml keeps its pull_request check and has no `push` trigger; with pages selected, pages.yml carries the docs and no leg renders.

```text
checks + ci -> all-green -> post-green (repo-owned hook) -> release (release-please module) -> pages (pages module, ordered behind release only where it exists)
                        \-> pages (pages module, without release-please)                     \-> docs-site (docs-site module, ordered behind post-green, and release where it exists)
```

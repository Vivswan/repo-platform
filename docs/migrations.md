...
3. Add its case to the upgrade-path harness: remove the rung file from the synthetic old build and plant the pre-transition state in [upgrade_path_test.sh](https://github.com/Vivswan/repo-platform/blob/main/.github/scripts/ci/upgrade_path_test.sh), then assert the postcondition after the ladder runs in [upgrade_path/01_main_update.sh](https://github.com/Vivswan/repo-platform/blob/main/.github/scripts/ci/upgrade_path/01_main_update.sh) (the `migration-ladder` rule counts a case in the entry or in any leg the entry sources).

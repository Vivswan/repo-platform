...
3. Add its case to the upgrade-path harness: an entry in `PENDING_RUNGS` in [tests/ci/upgrade_path/rungs.ts](../tests/ci/upgrade_path/rungs.ts) whose `model` plants the pre-transition state in the synthetic old build (the harness removes the rung file itself), then assert the postcondition after the ladder runs in [tests/ci/upgrade_path/01_main_update.test.ts](../tests/ci/upgrade_path/01_main_update.test.ts) (the `migration-ladder` rule reads the table: every rung on the ladder has an entry, every entry names a rung).

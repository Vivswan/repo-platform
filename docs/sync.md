...
- **The checker surface** is `actions/validate-managed-files/` and `actions/shared/`: the fleet check's own code and its shared imports (`CHECKER_SURFACE` in [sync/writer/judged_commit.ts](../.github/scripts/sync/writer/judged_commit.ts)). The writer the check runs is not on it: a writer change that matters writes a byte and moves the stamp on its own, and one that writes nothing leaves the repository matching what C writes, so C still judges it.

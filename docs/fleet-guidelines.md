...
| [Pinned actions](#pinned-actions) | pinact in repo-platform's `actionlint` job (landing); zizmor `unpinned-uses` and `impostor-commit` in every fleet push and PR run; Dependabot bumps the pins |

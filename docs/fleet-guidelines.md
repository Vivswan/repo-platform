...
- Exception: an action that publishes no version tags is pinned to a branch commit with the branch in the comment, `uses: <owner>/<action>@<40-hex sha> # master`, and the version it installs travels as the step's explicit input. The rule's `BRANCH_PINNED` table records each such action with its reason (empty today: every action the fleet ships publishes tags); a moving tag is still refused.

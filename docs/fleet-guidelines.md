...
| [File size caps](#file-size-caps) | the `file-size` step (a hard cap fails the step, and the step fails the `base-checks` job) |
...
| file-size | base-checks | a hard-cap finding or an allowlist defect | the path in the repo-owned `.file-size-allow.local` with a `# reason` (every finding on that path); a `comment-cap: ignore <reason>` line inside or above a comment block ([file size caps](#file-size-caps)) |

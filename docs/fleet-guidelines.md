...
- Exception: an action that publishes no version tags is pinned to a branch commit with the branch in the comment, `uses: <owner>/<action>@<40-hex sha> # main`, the sha alone naming the version. The rule's `BRANCH_PINNED` table records each such action with its reason (today `Vivswan/skills`, whose validate-skills action repo-platform's own ci.yml runs on its skills catalog); a moving tag is still refused.

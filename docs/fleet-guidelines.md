...
| zizmor | zizmor | a high finding (a pass that fails runs once more, so one transient error in an online audit needs no re-run; a high finding fails both judging attempts) | a `# zizmor: ignore[rule]` comment on the finding's line with the reason beside it; a `rules.<rule>.ignore` entry naming the file in the repo-owned `.github/zizmor.yml` |

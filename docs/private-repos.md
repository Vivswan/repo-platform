...
- The sync PR in the target repo carries everything the public log hides: dropped conflict hunks, removed paths, drift values, withheld workflow files, and the tail tripwire's report (the quoted repository-owned lines a split file lost, which force the manual-review path). Its checks run in the private repo, where logs are private.

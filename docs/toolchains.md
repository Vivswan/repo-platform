...
- repo-platform's own composite actions (under `actions/`) pin their bun too, from a generated action-local `.bun-version` beside each action.yml:
  - Each action's setup steps probe for a bun already at the pin, install it with oven-sh/setup-bun otherwise (one retry for the known network flake), and record the absolute path of the bun that prints the pin; every later step runs that path, never `bun` by name, because a later setup-bun can put another bun first on PATH.
  - `actions/bun-setup` is that sequence as one shared composite action: `pin` (the caller's .bun-version path) in, `path`, `ready`, and `installed` out; `required: false` leaves a missing bun to the caller through `ready`. The actions still carry the sequence inline: a composite action can only reference it as the published `@build` action, so it ships first and the actions adopt it in the follow-up.
  - The actions run vendored scripts and action-local lockfiles inside caller checkouts, so the CALLER's version resolution must never pick their bun: a repository pinning an older bun cannot parse the lockfiles repo-platform's bun writes. The actions-bun-guard ssot rule demands exactly one bun setup per action ahead of any other step that uses an action or touches bun, keeps every setup-bun step reading its pin (the shared action's `pin` input, the action-local dotfile, or a tree the action fetched itself), refuses a dangling `steps.<id>.outputs.path`, and refuses a bare `bun` line.

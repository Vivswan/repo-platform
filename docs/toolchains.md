...
- repo-platform's own composite actions (under `actions/`) pin their bun from the same file: a `uses:` fetch is the whole repository at the ref, so the pin path climbs from the action's directory to the repository root.

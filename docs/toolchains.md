...
- repo-platform's own composite actions (under `actions/`) pin their bun too, from a generated action-local `.bun-version` beside each action.yml (their setup steps pass `bun-version-file` against `github.action_path`). They run vendored scripts and action-local lockfiles inside caller checkouts, so the CALLER's version resolution must never pick their bun: a repository pinning an older bun cannot parse the lockfiles repo-platform's bun writes. `bun run generate` emits the dotfiles from the same bun manifest pin, and the actions-bun-guard ssot rule keeps every setup step reading them. After the setup, one step records the installed bun's absolute path and every later step runs that path, never `bun` by name (a later setup-bun can put another bun first on PATH); the rule refuses a bare `bun` line. The script directories under `actions/` that ship no action.yml of their own (generate.ts's `PINNED_SCRIPT_DIRS`; today the validator nested at `actions/validate-template-report/validator`) carry the same generated pin: validate-template-report runs its validator on its own bun, and a fetched build tree's copy runs on the pin that tree carries.

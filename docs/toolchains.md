...
- repo-platform's own composite actions (under `actions/`) pin their bun too, from a generated action-local `.bun-version` beside each action.yml (their setup steps pass `bun-version-file` against `github.action_path`). They run vendored scripts and action-local lockfiles inside caller checkouts, so the CALLER's version resolution must never pick their bun: a repository pinning an older bun cannot parse the lockfiles repo-platform's bun writes. `bun run generate` emits the dotfiles from the same bun manifest pin, and the actions-bun-guard ssot rule keeps every setup step reading them. The script directories under `actions/` that ship no action.yml (branch_tree.ts's `PINNED_SCRIPT_DIRS`; today `validate-template`) carry the same generated pin: validate-template-report runs the validator as its sibling on its own bun, and a fetched build tree's copy runs on the pin that tree carries.
- Each action probes for that pin with one canonical resolver block before and after its setup: a bun counts as pinned only when an ABSOLUTE executable on PATH prints exactly the pinned version AND exits 0 (a bun that prints the version but dies is no bun, so setup runs), and every later step runs the bun by the absolute path the post-setup resolver recorded, never `bun` by name (a later setup-bun can put another bun first on PATH). The actions-bun-guard ssot rule covers every action not declared bun-free (a declared one may not so much as mention bun): it pins the block's text and holds every other run step to a small grammar (privileged bash, `bash --noprofile --norc -p -eo pipefail {0}`, under which a caller's BASH_ENV, SHELLOPTS and env-exported functions are ignored; each line a recorded-bun invocation, the guard around it, a plain assignment, a printf with a static format, or a scratch removal), so bun can be run no way but by the recorded path; longer logic belongs in a script the recorded bun runs.

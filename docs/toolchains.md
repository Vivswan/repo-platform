...
- repo-platform's own composite actions (under `actions/`) pin their bun too, from a generated action-local `.bun-version` beside each action.yml, and the setup steps reading it are generated as well:
  - `bun run generate` emits the dotfile from the same bun manifest pin and fills each action.yml's fenced `bun-setup` region (scripts/action_bun_setup.ts is the source) with the pin probe, the pinned install (`bun-version-file` against `github.action_path`), one retry, and the step recording the installed bun's absolute path; every later step runs that path, never `bun` by name (a later setup-bun can put another bun first on PATH).
  - validate-template-report carries the `bun-setup-ready` variant: its retry may fail and the resolver exports `ready`, so the report step still runs and renders a missing bun instead of the action dying before it.
  - The actions run vendored scripts and action-local lockfiles inside caller checkouts, so the CALLER's version resolution must never pick their bun: a repository pinning an older bun cannot parse the lockfiles repo-platform's bun writes.
  - `bun run generate:check` holds each region's content; the actions-bun-guard ssot rule demands the region of every action that runs bun, keeps any extra setup-bun step reading the action-local pin (or a tree the action fetched itself), and refuses a bare `bun` line.

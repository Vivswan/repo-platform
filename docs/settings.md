...
- always: `dependencies` (`0366d6`) and `github_actions` (`000000`) - dependabot recreates its labels when missing, so an undeclared one would loop delete/recreate nightly - plus the triage trio `bug`, `enhancement`, `fix-lint`, the owner's approval label `merge-when-green` (whoever runs the landing merges a PR carrying it once every check is green), and the fleet-wide `security-nightly` stream label

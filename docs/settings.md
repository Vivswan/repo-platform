...
| 6 | Fleet override | [.github/settings-override.yml](../.github/settings-override.yml) | the invariants no repo may weaken: the squash-only merge policy, `allow_auto_merge`, `enable_vulnerability_alerts`, and the `main` and `non-bypassable` protection rulesets |
...
- private repos only: the `settings-as-code-report` marker label (`0e2a47`) that private reporting pins its report issue with, declared in the private visibility overlay - the central apply injects it automatically for redacted targets, but the repository's own self-apply runs unredacted and injects nothing, so the overlay declares it
...
Every render also carries the managed `settings-sync.yml` workflow (push to main touching that file, plus manual dispatch), which self-applies through [reusable-apply-settings.yml](../.github/workflows/reusable-apply-settings.yml)`@build` - the green-gated delivery branch, like every other fleet-rendered reference. The build tree carries only workflow files, so the reusable two-hops for its scripts: it reads the build tip's provenance stamp and checks out repo-platform at the stamped green main commit ([build-provenance.md](build-provenance.md)), then computes the same baseline + merge, so a partial repo document can never delete baseline labels. It runs the same two rechecks the central apply does - the registration file in the checkout, and the default-branch head immediately before the apply - so a self-apply queued behind a later push cannot write a stale document.
...
A per-repo PAT is only needed for the repository's self-apply-on-push, and only needs Administration and Issues on that one repository ([create a self-apply PAT with those pre-selected](https://github.com/settings/personal-access-tokens/new?name=REPO_PLATFORM_TOKEN&description=settings-sync+self-apply&administration=write&issues=write)); the fleet link's extra scopes (Contents, Pull requests, Workflows) are for push sync and are not needed here.

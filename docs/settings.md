...
- what the next apply would TAKE AWAY is CARRIED into the starter rather than left to a PR-body warning: the repo-local LABELS no fleet layer supplies (a name no layer ever rendered can only be the repository's own), plus every explicit `null` in the `labels` and `rulesets` sections - the dialect's "do not manage this", which dropping would re-arm. A label name the fleet already supplies is never copied: the fleet entry keeps it alive and a copy would only shadow it, so a restyle of a fleet label stays a reported drop.

...
- Tracking labels are the one non-file input: the label NAME is the repo's `labels.fuzzer` / `labels.nightly` / `labels.docs_site` registration value (the recorded `<key>_label` answer while the answers file exists; both present must agree), its color and description live in the module manifest, and the assembly appends it after merging the layers. The resolution is the plan action's own `trackingLabels` ([actions/plan/plan.ts](../actions/plan/plan.ts)), so a label the registration leaves unset is the module default files.yml declares, and the plan's refusals (a `labels` key naming no selected stream, a name the settings layers already manage, two streams sharing one name) fail that repo's apply the way they fail its CI.
...
- A stream module (fuzzer, nightly, docs-site) selected in `.repo-platform.yml` without a `labels.<key>` entry gets the module's default label; the repository renames the stream by adding the entry, and the next apply deletes the default label as undeclared and creates the declared one. A name that collides with another stream's label fails the apply like any label collision.

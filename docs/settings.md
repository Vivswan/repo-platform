...
Every render carries `.github/settings.yml` once (base content): the four identity keys seeded from the copier answers (`homepage` and `topics` are asked of every repository), plus commented examples for local labels and rulesets. It is repo-owned from then on (`_skip_if_exists`).

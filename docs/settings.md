...
- what the next apply would change under the repo is CARRIED into the starter rather than left to a PR-body warning: the repo-local LABELS no fleet layer supplies (the apply deletes undeclared labels), the RULE TYPES the old file added to a ruleset the fleet also declares (the apply PUTs a declared ruleset whole, so an omitted type leaves the live ruleset), and every explicit `null` in those sections - the dialect's "do not manage this", which dropping would re-arm. Names the fleet already supplies are never copied - the fleet entry keeps them alive and a copy would only shadow it - so a restyle of a fleet label or rule stays a reported drop. A repo-only RULESET is not carried either: the apply never deletes an undeclared ruleset, so its live protection survives and re-declaring it is the reviewer's call.

...
Leaving management is the only opt-out: revoke the fleet token's write access to the repo (a private repository then disappears from the nightly heal's discovery; a public one stays listed, and every heal whose scope includes public repositories prints one "not in the fleet" notice for it; nothing is deleted either way) or delete its `.repo-platform.yml`. The repo's `settings.yml` stays either way (sync never deletes it) as inert documentation or for hand use.

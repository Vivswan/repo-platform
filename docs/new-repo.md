...
| `.github/repo-platform-manifest.json` | The ownership manifest: each template-landed path's class (`managed`, `split`, or `starter`) plus sha256 hashes of the managed content, stamped after each render. validate-template's INTEGRITY check blocks on drift against it: managed content changed outside a sync, a listed managed file missing from the repo, or a roster path the manifest does not list. One state is an advisory instead: a `.github/workflows/` entry with a null hash and no file, the state a withheld workflow leaves (the sync token lacked the Workflows scope; a hand-deleted workflow the template did not change looks the same after a sync restamps). Its freshness report never blocks. |

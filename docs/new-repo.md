...
- a run in which release-please creates or refreshes the release PR (independent of any release cut; a push with no releasable commits triggers neither) calls the repo-owned `update-release-pr.yml` hook with the PR's number and head branch: regenerating files that must ride in the release commit and updating version references in docs go there. Pushes it makes to the PR branch with the default `GITHUB_TOKEN` do not re-trigger the PR's checks; with `REPO_PLATFORM_TOKEN` they do.

...
- Every target is recorded in the ownership manifest as class `mirror` with the copy's hash, so validate-template reds on a drifted or deleted copy like any managed file. A declared literal target with no record must hold its source's bytes: a declaration whose copy was never written is red until a sync writes it, or the declaring PR copies the source there itself (the next sync adopts and records the copy). The implementation: [writer/mirrors.ts](https://github.com/Vivswan/repo-platform/blob/main/.github/scripts/sync/writer/mirrors.ts) and, for the rules both readers share, [actions/plan/mirrors.ts](https://github.com/Vivswan/repo-platform/blob/main/actions/plan/mirrors.ts).

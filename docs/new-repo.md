...
| File | Role |
|---|---|
| `.repo-platform.yml` | The module selection's home: edit its `modules:` list in a PR; the sync PR carrying the module's files follows the merge ([changing the module selection](#changing-the-module-selection)). Its presence is what marks the repo as managed. Repo-owned: the sync reads it and rewrites it only on the cutover of a repository registered the old way ([sync.md](sync.md#cutover)). |
...
| deno | A managed `deno-audit.yml` that runs `deno audit` weekly, on lockfile-touching PRs, and on pushes to main that change `deno.lock`, failing when any locked dependency (JSR or npm, transitive included) has a high or critical advisory. Every tracked `deno.lock` is audited, nested workspace lockfiles included. |

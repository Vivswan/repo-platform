...
- bun lockfile fixes, this PR: the new head's `pull_request` run sits at "awaiting approval". Open it in the Actions tab and choose "Approve and run", or push an empty commit. On a public repository, whose ruleset carries the code-scanning rule, a hand `workflow_dispatch` run does not unblock the merge: it wants the PR-event CodeQL analysis.

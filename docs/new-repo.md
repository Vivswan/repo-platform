...
- `.github/workflows/ci.yml` is template-managed: the standard jobs
  ...
  per-language `codeql-*` jobs on public repos with a toolchain), the
  aggregate `all-green` gate, and a `checks` job that calls checks.yml.
  Sync updates it; don't edit it directly.

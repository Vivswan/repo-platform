...
- The render also carries a `settings-sync.yml` workflow for push-time self-apply (needs a repo-scoped PAT; warns and skips without one); the central heal applies the repo's settings either way.

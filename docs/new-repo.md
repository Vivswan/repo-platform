...
- The release is cut by the run on the release commit: `github.token` cannot tag an older commit once a later commit changed a workflow file on main (that ref creation needs `workflows: write`, which it never holds); `REPO_PLATFORM_TOKEN` can.

...
| `.github/workflows/post-green.yml` | repo-owned (a starter, written once) | the repository's own green-gated work (applying settings, refreshing generated artifacts): the managed `post-green` job calls it on every push to main whose gate passed, with the judged sha, before the release leg ([after the gate](all-green.md#after-the-gate)). The caller grants `contents: write` (a fast-forward branch push) and `id-token: write` (OIDC trusted publishing or attestations), the ceiling for every hook job. Seeded as a no-op |

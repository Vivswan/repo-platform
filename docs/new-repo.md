...
| | |
|---|---|
| Rule | No file over its hard line cap, no code line over 256 characters: source 2000 lines, tests 3200 (a source file named `*.test.*`, `*_test.*`, `*.spec.*`, `*_spec.*`, `test_*`, Rust's `*_tests.rs`, `tests.rs`, `proptests.rs`, or under `test/`, `tests/`, `__tests__/`), workflow yaml and `action.yml` 1000, shell 1000, markdown 1300. The warn tier (80% of each line cap: 1600/2560/800/800/1040; width 150) annotates without failing; a warn-wide line that is one string or regex literal (assigned, returned, or keyed) is left alone, since wrapping it means splitting the literal. |

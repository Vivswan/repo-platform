...
- `source` names one file this sync wrote (class `managed` or `split`); mirroring repo-owned content is the repository's own job.
...
- Targets must stay inside the repository, must not be platform-written paths themselves (one path, one writer), and neither side may sit under `.github/workflows/` (workflow files are platform-written, managed or written-once starters, so a mirror there would be a second writer). No two sources may claim one target and no target may be a path prefix of another; both sides of either conflict are refused, so declaration order never chooses the winner. A symlink at either side's path or among its ancestors is refused too: following one would carry the read or the write outside the checkout.
...
- Mirror targets are recorded as class `mirror` (the hash of the copy, so the next sync can tell its own previous write from a local edit), and a target that is itself a `files.yml` path is refused. The implementation: [sync/writer/mirrors.ts](../.github/scripts/sync/writer/mirrors.ts); [sync.md](sync.md#mirrors) has the outcome table.

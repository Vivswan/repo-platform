...
- CI gates on the `all-green` status check, which needs every gating CI job, plus Copilot's own `copilot-pull-request-reviewer` review check (the convention is documented in [repo-platform's all-green guide](https://github.com/vivswan/repo-platform/blob/main/docs/all-green.md)).
...
This repository is the template source itself: `bun run check` chains every local gate, and `CLAUDE.md` documents the editing rules and smoke-generate recipe.

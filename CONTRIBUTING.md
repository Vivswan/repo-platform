...
- CI gates on the `all-green` status check - a check run a verdict workflow creates after judging every gating CI job of a run - plus Copilot's own `copilot-pull-request-reviewer` review check (the convention is documented in [repo-platform's all-green guide](https://github.com/vivswan/repo-platform/blob/main/docs/all-green.md)).

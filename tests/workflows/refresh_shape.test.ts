...
const ROWS: Row[] = [
  {
    ...
    runs: [
      ...
      "bun .github/scripts/fleet/require_fleet_token.ts",
      "bun scripts/generate/build_gitignore.ts",
    ],
...
test.each(ROWS)(
  "$workflow checks out with the default token, requires the fleet token, then opens its PR through create-pull-request alone, unconditionally, as the sync identity",
  ({ workflow, runs, branch, title, body }) => {
    const file = `.github/workflows/${workflow}.yml`;
    const source = readFileSync(join(ROOT, file), "utf8");
    const doc = parseYaml(source) as Workflow;
    const [checkout, ...steps] = doc.jobs.refresh.steps;
    expect(doc.permissions).toEqual({ contents: "write", "pull-requests": "write" });
    expect(checkout.uses).toMatch(/^actions\/checkout@/);
    expect(checkout.with).toEqual({ ref: "${{ github.event.repository.default_branch }}" });

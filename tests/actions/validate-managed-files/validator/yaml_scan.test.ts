...
describe("duplicate mapping keys", () => {
  test.each([
    ...
    {
      reason: "a document of a multi-document stream (itself a second error)",
      path: ".github/multi.yml",
      content: "a: 1\na: 2\n---\nb: 3\n",
    },
    { reason: "a fixture outside .github/", path: "tests/fixtures/dup.yml", content: DUP_KEY_YAML },
    {
      reason: "a multi-document file outside .github/",
      path: "deploy/manifests.yml",
      content: "a: 1\na: 2\n---\nb: 3\n",
    },
  ])("a duplicate key anywhere fails with the tailored message: $reason", ({ path, content }) => {
    ...
    expect(stdout).not.toContain("advisory");
  });
...
describe("YAML syntax errors", () => {
  test.each([
    ...
    {
      reason:
        "a malformed directive with no document behind it (the error sits on the stream, not on a document)",
      path: "config.yml",
      content: "%TAG\n",
    },
    {
...
describe("multi-document YAML", () => {
  test.each([
    { reason: "a .github/ file", path: ".github/dependabot.yml" },
    { reason: "a file outside .github/", path: "deploy/manifests.yml" },
  ])("a VALID multi-document stream fails - consumers read one mapping: $reason", ({ path }) => {
    const { exitCode, stderr } = runValidator({
      [path]: "kind: Service\n---\nkind: Deployment\n",
    });
    expect(exitCode).toBe(1);
    expect(stderr).toContain(`${path}: multi-document YAML stream (2 documents)`);
  });

  test("an explicit document start is one document", () => {
    const { exitCode, stderr } = runValidator({ "deploy/service.yml": "---\nkind: Service\n" });
    expect(stderr).toBe("");

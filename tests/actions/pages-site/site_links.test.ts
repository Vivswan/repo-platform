...
describe("fragment reads", () => {
  ...
  test("fragmentIds collects element ids and anchor names, and top always resolves", () => {
    expect(
      fragmentIds(
        '<h2 id="install">I</h2><a name="legacy"></a><div id="">x</div><p id="VPContent">',
      ),
    ).toEqual(new Set(["top", "install", "legacy", "VPContent"]));
  });
});
...
describe("checkSiteLinks", () => {
  ...
  test("passes a site whose absolute (based), relative, directory, and fragment links all resolve", async () => {
    const dir = site({
      "index.html":
        '<a href="/r/docs/">docs</a> <a href="/r/docs/latest/setup.html#install">i</a>' +
        ' <a href="https://example.test/">out</a>',
    ...
    const result = await checkSiteLinks(dir, "/r/", tiers, temp.dir("site-links-scratch-"));
    expect(result.pages).toBe(3);
    expect(result.judged).toBeGreaterThan(0);
  });

  test("fails on a missing target or fragment from a current page, listing page -> link, and ignores history's", async () => {
    const dir = site({
      "index.html": '<a href="/r/docs/skills/missing/">m</a>',
    ...
    try {
      await expect(
        checkSiteLinks(dir, "/r/", tiers, temp.dir("site-links-scratch-")),
      ).rejects.toThrow("2 broken internal links in the current content");
  ...
  test("a site without a current page is failed-to-look", async () => {
    ...
    await expect(
      checkSiteLinks(dir, "/", [{ rel: "v1/", strict: false }], temp.dir("site-links-scratch-")),

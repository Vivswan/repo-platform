...
// Records every git/gh invocation to CALLS_LOG; `gh pr list` answers the
// canned PR_LOOKUP, and GIT_FAIL/GH_FAIL pick a command that exits 1.
const stub = (tool: string) => `#!/usr/bin/env bash
set -euo pipefail
{ printf '${tool}'; printf ' %s' "$@"; echo; } >>"$CALLS_LOG"
...
describe("open_automation_pr.ts", () => {
  ...
  test("an existing PR gets its body refreshed, not a duplicate PR", () => {
    ...
    expect(r.exitCode).toBe(0);
    expect(r.calls).toContain("gh pr edit 17 --body Automated refresh body.");
  ...
  test("REFRESH_TITLE=true refreshes an existing PR's title too", () => {
    ...
    expect(r.calls).toContain(
      "gh pr edit 17 --title chore: refresh x from upstream --body Automated refresh body.",
    );
  });

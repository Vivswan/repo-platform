...
describe("resolve_action_refs.ts over a scratch tree", () => {
  ...
  test("a stale comment, a dangling ref, a comment naming no release, and an API failure each fail with their own message; the sound pin beside them stays silent", () => {
    ...
    expect(result.stderr.trimEnd().split("\n")).toEqual([
      ...
      `::error::action-refs: actions/setup-node@v7.0.999 (the comment beside ${DEAD}) does not resolve to any commit, tag, or branch upstream (pinned in files/m/.github/workflows/c.yml.block.toolchain). Name the release the pinned sha is.`,

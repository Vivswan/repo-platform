...
describe("sync.ts end to end", () => {
  ...
  test("selects the known modules and drops the unknown one into a note", () => {
    expect(summary.modules).toEqual(["bun", "docs-site", "fuzzer"]);
    expect(summary.notes).toEqual(["dropped unknown module `uv` (files.yml does not know it)"]);
  });
  ...
  test("never touches the existing starter", () => {
  ...
  test("mirrors the written license into each skill, refusing the foreign copy", () => {
  ...
  test("a second run changes nothing and reports every file unchanged", () => {
    const before = fixtureGit(target, ["status", "--porcelain"]);
    ...
    expect(fixtureGit(target, ["status", "--porcelain"])).toBe(before);
  });
});

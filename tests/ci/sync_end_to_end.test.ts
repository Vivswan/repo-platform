...
// The pre-render settings.yml: identity keys, a ruleset of its own, comment
// lines, a CRLF line, and no trailing newline, so the move is checked byte
// for byte.
const OLD_SETTINGS = [
  "---",
...
function oldManifest(): string {
  const entries: Record<string, string> = {
    ...
    // The settings starter of the previous pipeline, displaced by the render.
    [SETTINGS]: `{"class": "starter"}`,
...
describe("sync.ts end to end", () => {
  ...
  test("the hand-written settings.yml moves verbatim to the overlay and the render replaces it", () => {
    expect(readFileSync(join(target, OVERLAY), "latin1")).toBe(OLD_SETTINGS);
  ...
  test("a second run changes no byte and reports every file unchanged", () => {
    ...
    // The displacement happened once: the render is now the recorded write.
    expect(again.summary.holdReasons).not.toContain(MOVED_REASON);
...
describe("sync.ts over a repository whose settings or overlay path is taken", () => {
  ...
  test.each<{
  ...
  }>([
    {
      ...
      holds: [`${SETTINGS} held: ${NOT_MOVED("a regular file")}`],
      still: (target: string) => {
    ...
    },
    {
      ...
      holds: [
        ...
        `${SETTINGS} held: ${NOT_MOVED("a directory")}`,
      ],
      still: (target: string) => {
        expect(readFileSync(join(target, SETTINGS), "latin1")).toBe(OLD_SETTINGS);
    ...
    },
    {
      ...
      // Never moved over the overlay; with no record vouching for its bytes
      // the re-render is shown as a replaced edit once, then recorded.

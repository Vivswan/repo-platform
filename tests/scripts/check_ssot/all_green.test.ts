...
import { neutralizePlaceholders } from "../../../scripts/check/ssot/inputs.ts";
...
describe("callerCeilingMismatches", () => {
  ...
  test("the live called workflows fit their skeleton callers; raising one scope goes red", () => {
    const skeleton = parseYaml(neutralizePlaceholders(readFileSync(SKELETON, "utf-8"))) as {
      jobs: Record<string, { permissions?: unknown }>;
    };
    for (const [rel, job] of Object.entries(FLEET_CALLERS)) {
      const text = readFileSync(rel, "utf-8");
      const site = { rel: SKELETON, job, permissions: skeleton.jobs[job].permissions };
...
describe("skeletonGateMismatches", () => {
  const source = neutralizePlaceholders(readFileSync(SKELETON, "utf-8"));
  ...
  test.each<{
    ...
    },
    {
      ...
      mutate: (j) => {
        j.ci.uses = "placeholder/repo-platform/.github/workflows/fleet-nightly.yml@build";
      },
      job: "ci",
    },
    {
    ...
    expect(found.every((m) => m.file === `${SKELETON} job '${job}'`)).toBe(true);
  });
  ...
  test("a skeleton missing a caller job is one loud mismatch per absent job", () => {
    ...
    expect(skeletonGateMismatches(mutated)).toEqual([
      { file: `${SKELETON} job 'nightly'`, expected: "the job present", got: "no such job" },
    ]);
  });
});

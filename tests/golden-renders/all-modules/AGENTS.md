...
- PR titles and commit subjects must be Conventional Commits (`feat:`, `fix:`, `feat!:`, `chore:`, ...). PRs are squash-merged, so the PR title becomes the commit subject and drives release-please versioning. CI validates both (fleet-ci's pr-title job + validate-commit-names).
- CI gates on the required check `all-green`: a check run the managed `.github/workflows/all-green.yml` verdict workflow creates after judging each CI run's jobs plus the expected Copilot review of the head (the gate jobs themselves run centrally through repo-platform's fleet-ci.yml; `all-green` is the ruleset's SOLE required check). This repository's own test/lint jobs belong in `.github/workflows/checks.yml` (repo-owned, called inside the gate); do not edit ci.yml or all-green.yml, template sync overwrites them. A green verdict on a push to main releases: the wrapper's `release` job (`needs: [verdict]`, gated on the verdict's posted conclusion) calls the managed release pipeline in `.github/workflows/release.yml` with the judged commit; this repository's release preparation (packaging, asset uploads, note edits) goes in the repo-owned `.github/workflows/update-release.yml` hook it calls.

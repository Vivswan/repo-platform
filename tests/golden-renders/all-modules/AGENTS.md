...
- Repository settings (description, topics, labels, rulesets, merge policy) are applied from Vivswan/repo-platform: it computes this repository's managed baseline (shared policy, module-derived labels, fleet rulesets) at apply time and deep-merges this repository's own `.github/settings.yml` (identity keys and local overrides; entries there replace same-name baseline entries wholesale) over it. Do not change settings by hand in the GitHub UI; edit `.github/settings.yml`.

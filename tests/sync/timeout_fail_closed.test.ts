...
// scrubbed for the whole file and restored after.
...
function initRepo(files: Record<string, string>): string {
  ...
  const run = (...args: string[]) => {
    const proc = capture(["git", "-C", dir, ...args]);
 ...
 * abort under test exits the DRIVER process, never this one. */
function runDriver(
  source: string,
  ...
  const proc = Bun.spawnSync([process.execPath, driver], {
    ...
    stdout: "pipe",
    stderr: "pipe",
    // The test's own backstop, generous next to PROBE_TIMEOUT_MS: a
    // driver that outlives it is itself a hang regression.
    timeout: 10_000,
    killSignal: "SIGKILL",
  });

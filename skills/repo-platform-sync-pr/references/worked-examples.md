...
## 7. `moved` on `.github/settings.yml`

The first sync after the rendered settings arrived. The Written rows read `.github/settings.local.yml`, `starter`, `unchanged` and `.github/settings.yml`, `managed`, `moved` with the detail `to .github/settings.local.yml`; the Review section holds once with `.github/settings.yml: the repository's file moved to .github/settings.local.yml and the rendered document replaced it`. The diff shows the old file renamed and a new `.github/settings.yml` opening with `# Generated by repo-platform - do not edit.`

```bash
git fetch origin main automation/repo-platform
git diff origin/main:.github/settings.yml origin/automation/repo-platform:.github/settings.local.yml
```

Resolution: expected. The command prints nothing (the move is verbatim, comments included). Read the rendered file as the merge of the fleet layers, the selected modules' layers, and your overlay; your labels and rulesets appear beside the fleet's. Merge; the next sync reports both rows `unchanged`, and the settings apply picks the repository up once this PR has merged.

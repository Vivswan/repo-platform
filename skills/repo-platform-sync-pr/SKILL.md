...
- Clean updates arm squash auto-merge: the PR merges itself once every required check passes - the fleet ruleset requires two: the `all-green` verdict check run and Copilot's own `copilot-pull-request-reviewer` check run (created when its code review runs on the current head). A PR stays manual-review when any of these hold: auto-resolved conflicts, a split-file carry needing review (an appendix, reset managed-half edits, duplicate markers), a tripped tail tripwire, failed validation, a recovery re-render, a forced-manual dispatch, a deleted license file, withheld workflow files, out-of-band settings drift, or a settings.yml layering transition that dropped overrides or failed.
...
- Green means the repo's required checks pass on the branch - under the fleet ruleset that is `all-green` plus Copilot's `copilot-pull-request-reviewer` check run (a red validate-template check flags drift to fix, but where it is not required it does not gate the merge).

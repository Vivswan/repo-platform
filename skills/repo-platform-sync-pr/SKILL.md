...
- The PR body warns about merge conflicts, dropped local lines, retired files, withheld workflow files, a new starter at a path the repo already owns, settings drift, or failed validation
...
| Workflow files WITHHELD warning | The fleet token lacks the Workflows scope; `.github/workflows` changes are missing from the diff, and an added workflow's manifest entry is stamped `"withheld": true` | Grant Workflows RW to REPO_PLATFORM_TOKEN in repo-platform and re-run the sync before merging; once the partial PR has merged, a plain update honours the committed absence, so dispatch a recovery sync (`recover=recopy`) to deliver the withheld files |

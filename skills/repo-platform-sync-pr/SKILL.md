...
| Registration notes | one line per note: an unknown module dropped, an unreadable manifest, a placeholder with no value, or a manifest record dropped or ignored (the exact forms below) | Fix the registration. An unreadable manifest is rewritten by this sync; a managed file or region that differs from the incoming content reads as replaced in the same report. A dropped mirror record means that copy is the repository's own now |

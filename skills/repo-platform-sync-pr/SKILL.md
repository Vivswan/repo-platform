...
- ``cutover: the `pages` module is the `site` module now, and its build is the repo-owned .github/actions/site-build/action.yml hook: select `site` and move <build> into the hook before merging`` (`<build>` is the recorded `pages_*` answers, or `its build` when none was recorded), and ``cutover: the `docs-site` module is the `site` module now: select `site`<settings> before merging`` (`<settings>` names the `site.path` or `labels.site` key a non-default answer becomes): the old registration selected a module that became `site`; edit `.repo-platform.yml` as the note says, in its own PR
- `site cutover: .github/workflows/pages.yml is retired and .github/actions/site-build/action.yml is seeded as a no-op; move the former pages build into the hook before merging, or the next main run serves the docs alone (or nothing)`: the retired deploy is still in the tree and no hook exists yet; the note returns every sync until the hook does (the Site cutover row below)
...
| Site cutover | The `pages` and `docs-site` modules became `site`: Retired rows `deleted` for `.github/workflows/pages.yml` and `docs-site.yml`, a Written row `created` for the starter `.github/actions/site-build/action.yml` (the no-op site-build hook), and for a repo that had `pages.yml` a `site cutover:` Registration note holding the PR | Every former `pages` repo had a website build: `pages.build`, or its toolchain's default (`bun run build` into `dist` for bun). Move that build into the hook on the PR branch BEFORE merging (`dist` names the built directory), or the next main run serves the docs alone (or nothing); the note returns every sync until the hook exists. A hook the repo already had reads `unchanged`: check it declares the `base-path` and `origin` inputs and the `dist` output. The registration itself needs `site` selected and no `pages:` or `docs_site:` block, in its own PR |

...
- Green means the repo's required checks pass on the branch - under the fleet ruleset that is `all-green`, whose verdict also waits for Copilot's review of the head (a red validate-template check flags drift to fix, but where it is not required it does not gate the merge).

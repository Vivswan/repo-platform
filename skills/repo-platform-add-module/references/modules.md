...
- Companion steps:
  - Settings labels are automatic: the managed baseline declares `dependencies` (`0366d6`) and `github_actions` (`000000`) always, plus the toolchain label - `javascript` (`168700`) for bun/node, `deno` (`70ffaf`), `python:uv` (`2b67c6`), `rust` (`000000`) - from the module manifests (`templates/<module>/module.yml`).
...
- Settings labels are automatic: the managed baseline declares `autorelease: pending`, `autorelease: tagged`, `release-blocker` (`B60205`), `release-override` (`FBCA04`) from the module manifest, plus the `release-tags` tag-immutability ruleset.

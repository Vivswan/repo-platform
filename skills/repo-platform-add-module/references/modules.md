...
- The hook is a base starter every repository carries, seeded as a no-op (output `dist` empty: the docs publish alone). Fill it in with the website's build: inputs `base-path` and `origin`, output `dist` naming the built directory. repo-platform's `docs/site.md` has the contract and examples.
- Keys: `site.path` (URL segment the docs mount under beside a website; default `docs`), `site.include` (extra trees rendered into the docs: `{path, mount, page?}`), `labels.site` (link-rot tracking label; default `docs-link-rot`).

...
- Labels: the module's labels leave the rendered `.github/settings.yml` on that sync (`managed`, `updated`) unless another selected module still declares them (bun and node share `javascript`), and the next apply removes them from the repo.

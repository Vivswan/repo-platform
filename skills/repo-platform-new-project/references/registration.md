...
| `except` | Paths the repository keeps as its own: no `files.yml` entry at one is written for it, whatever its `when`, and a record there is retired like a deselected module's. A path no entry writes holds the sync PR with a Registration note | none |

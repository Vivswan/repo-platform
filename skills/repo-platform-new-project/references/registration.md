...
The registration is the only file a repository writes to be managed. The sync and fleet CI read it; the platform never rewrites it. Unknown keys, wrong types, and a missing `modules` list are refused by the `plan` job on every PR and by the sync.
...
| `pages.setup` | Comma-separated toolchain tokens the Pages build installs (`bun`, `uv`, ...), or `none` | the selected toolchain modules, joined by commas |
| `pages.install` | Install command of the Pages build | the install command of the first `pages.setup` toolchain in roster order |
| `pages.build` | Build command of the Pages build; must be nonempty when `pages` is selected | the build command of the first `pages.setup` toolchain in roster order |
...
- A repo that still carries that answers file must record the same label value there (`nightly_label`, `fuzzer_label`, `docs_site_label`) or the plan fails: the two must agree while both files exist.

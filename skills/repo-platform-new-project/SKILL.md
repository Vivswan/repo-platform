...
- After step 7's settings apply: the first PR shows `all-green` as the required check, and it passes once Copilot's automatic review of the head posts (the verdict waits for it). (Before the apply, `all-green` runs but nothing marks it required - that is expected, not a platform failure.)
